...
const xlsx = resolve(process.argv[2] ?? join(root, 'data/H2C_FDM_Material_Database.xlsx'));
const refXlsx = resolve(process.argv[3] ?? join(root, 'data/Generic_Materials_Reference.xlsx'));
...
writeFileSync(join(dataDir, 'tables/SOURCE.json'), JSON.stringify({
  note: 'Converted from the workbooks below by scripts/migrate/dump-workbook.mjs. The workbooks are retired; their history is in git.',
