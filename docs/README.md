...
2. **[DECISIONS.md](DECISIONS.md).** Numbered D1 to D43, each saying what would break if it were
   reversed, followed by a table of bugs that shipped and what pins each one now.
3. **[audits/](audits/).** Nine audit passes so far, each in its own dated folder: the report as it was
