...
| `VOCAB-NEAR-DUPLICATE` | lint | Values that differ only in case, spacing or punctuation in a short-list column; pick one spelling. | Fix it, or accept it with a reason: npm run data:lint -- --accept CODE "reason". |
...
| `SOURCE-UNCITED` | lint | A source no record cites; cite it, or say why it is registered. | Fix it, or accept it with a reason: npm run data:lint -- --accept CODE "reason". |
