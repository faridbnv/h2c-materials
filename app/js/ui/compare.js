...
export function renderCompare(host, state, actions) {
  ...
  const picked = scenario.shortlist.map((id) => db.materials.find((m) => m.id === id)).filter(Boolean);
  ...
  if (picked.length < 2) {
    host.innerHTML = `<div class="empty">
      ...
      ${picked.length === 1 ? `<strong>${esc(picked[0].name)}</strong> is pinned so far.` : ''}</p></div>`;
    return;
  }
  ...
  const blocks = AXIS_DEFS.map((a) => {
    ...
    if (!known.length) {
      return `<div class="cmp-prop"><h4>${esc(a.label)} <span class="unit">${esc(a.unit)}</span></h4>
    ...
    return `<div class="cmp-prop">
      <h4>${esc(a.label)} <span class="unit">${esc(a.unit)}</span></h4>
      ${picked.map((m, i) => {
        const h = m.headline[a.key];
        if (!h?.known) return `<div class="cmp-bar"><span>${esc(m.name)}</span>
  ...
  host.innerHTML = `
    <div style="display:flex;gap:10px;align-items:center;margin-bottom:16px;flex-wrap:wrap">
      ${picked.map((m) => `<span class="pin">${esc(m.name)}</span>`).join('')}
